...
# nawk 'BEGIN{FS=" "}{print $1}' data/xwnd-30g/biology.ppv | sort > data/synsets.txt

import os, subprocess
import textwrap, argparse

parserarg = argparse.ArgumentParser(
     prog='ponderateListSynsets',
     formatter_class=argparse.RawDescriptionHelpFormatter,
     description=textwrap.dedent('''\
         calculate ponderated vectors, from a list of synsets (synsets.txt)
         --------------------------------
...
for lin in infile.read().splitlines():
	...
	cmd = "fgrep "+lin+" data/wei_eng-30_variant.tsv | nawk '{print $1$2}'"
	...
	cmd = "fgrep "+lin+" data/xwnd-30g/*.ppv | nawk '{s=s+$2}END{print s}'"
	if debug: sys.stderr.write("\tCMD: "+cmd+"\n")
	...
	cmd = "fgrep "+lin+" data/xwnd-30g/*.ppv | nawk '{s=s+$2/"+str(aw)+"*log($2/"+str(aw)+")}END{print -s}'"
	if debug: sys.stderr.write("\tCMD: "+cmd+"\n")
	...
	cmd = "fgrep "+lin+" data/xwnd-30g/*.ppv | nawk '{n=$2*1/"+str(entropy)+"; print $1,$2,n}' | nawk '{s2=s2+$2;s3=s3+$3}END{print s2,s3}'"
	if debug: sys.stderr.write("\tCMD: "+cmd+"\n")
	...
	cmd = "fgrep "+lin+" data/xwnd-30g/*.ppv | nawk '{n=$2*1/"+str(entropy)+"; print $1,$2,n}' | sed 's/.*\///' | tr ':' ' ' | sort | join - data/xwndg.txt | sort -k 4gr | nawk '($4/"+str(wn)+")>"+str(threshold)+"{print $1, $2, $3, $4, $5, $4*log($5)}' | sort -k 6gr"
	if debug: sys.stderr.write("\tCMD: "+cmd+"\n")
